...
const FileDataPreview: React.FC<Props> = ({
  ...
  const renderFileData = (fileRef: FileDataRef, index: number) => (
    <div
      key={index}
      className="file-data-tile text-token-text-primary group relative inline-block text-sm"
    >
      <div className="border-token-border-light relative overflow-hidden rounded-xl border">
        <div className="h-14 w-14">
          ...
          >
            <span
              className="flex h-full w-full items-center justify-center bg-gray-100 bg-cover bg-center text-white dark:bg-gray-900"
      ...
      {!readOnly && (
        <Tooltip title="Remove file" side="top" sideOffset={0}>
          <button
            ...
            className="remove-file-button absolute right-1 top-1 -translate-y-1/2 translate-x-1/2 rounded-full border bg-white p-0.5 dark:bg-black md:opacity-0"
          >
    ...
    createPortal(
      ...
      >
        <div className="relative">
          {viewedFileIndex > 0 && (
            <button
              className="absolute left-4 top-1/2 flex -translate-y-1/2 items-center justify-center rounded-full border border-black bg-white bg-opacity-50 shadow-lg"
          ...
          {viewedFileIndex < fileDataRef.length - 1 && (
            <button
              className="absolute right-4 top-1/2 flex -translate-y-1/2 items-center justify-center rounded-full border border-black bg-white bg-opacity-50 shadow-lg"
              style={{ width: '48px', height: '48px' }} // Adjust size as needed
              onClick={(e) => {
                e.stopPropagation();
                handleNextPrev('next');
