    ...
    return (
      <div className={`relative ${className}`}>
        <textarea
          ...
          className="focus:shadow-outline w-full flex-1 resize-none appearance-none overflow-y-auto border border-gray-300
                    px-3 py-2 leading-tight text-gray-700 shadow
                    focus:outline-none dark:border-gray-600 dark:bg-gray-700 dark:text-gray-300"
          spellCheck={false}
        ></textarea>
        <button
          onClick={toggleModal}
          className="absolute bottom-2 right-2 rounded-full bg-gray-200 p-2 dark:bg-gray-800"
          aria-label="Expand"
        >
          <span>
        ...
        {isModalOpen && (
          <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50">
            <div className="flex h-4/5 w-4/5 flex-col rounded-lg bg-white p-4 shadow-lg dark:bg-gray-800">
              <textarea
                ref={textarea2Ref}
                id="instructions-fullscreen"
                name="instructions-fullscreen"
                className="w-full flex-1 resize-none border border-gray-300 p-2 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-300"
                defaultValue={currentValueRef.current}
                onChange={handleChange}
                spellCheck={false}
              ></textarea>
              <div className="mt-4 flex justify-end space-x-4 px-8">
                <Button
                  onClick={toggleModal}
                  variant="secondary"
                  className="mr-2"
